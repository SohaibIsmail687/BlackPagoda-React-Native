...
check_previous_payout = () => {
  ...
  let api = Connection + "rest_apis.php?action=check_previous_payout";
  ...
  fetch(api, {
    ...
    alert("You already have pending payout request.")
      }
    })
    .catch((error) => {
      console.error(error);
...
add_payout_request = () => {
    ...
    uploaddata.append("account_no",account_no);

    uploaddata.append("name", name);
  
 
    uploaddata.append("doctor_name", this.state.doctor_name);
    ...
    let api = Connection + "rest_apis.php?action=add_payout_request";
    console.log("pass => ", api);
    fetch(api, {
      method: "POST",
      headers: {
      ...
      .then((response) => {
        ...
        if (record == "fail") {
          ...
          // Toast.show(this.props.You_successfully_requested_payout);
          // this.notification()
          Actions.pop({  });
          // Actions.pop({  });
        }
        ...
        return (
            <View style={{ flex: 1, backgroundColor: 'white' }}>
                ...
                <View style={{ paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 15, shadowOffset: { width: 0, height: 2, }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, backgroundColor: 'white' }}>
                    ...
                    <Text style={{ color: 'black', fontSize: 15, fontWeight: '700' }}>Payout Request</Text>
                    <Text>       </Text>
                </View>
<ScrollView>
                <Text style={{ fontSize: 16,   color: 'gray', paddingHorizontal: 15, marginTop: 10 }}>Total Amount</Text>

                <Text style={{ color: 'black', paddingHorizontal: 17, fontSize: 16,fontWeight:'bold' }}>${this.props.total}</Text>

              
                <Text style={{     color: 'gray', paddingHorizontal: 15, marginTop: 10 ,marginBottom: 10,}}>Please enter your corect information for payout request</Text>
...
<Text style={{ color: 'black', fontSize: 13, paddingHorizontal: 15, marginTop: 10,fontWeight:'bold' }}>Amount</Text>

<TextInput
  placeholder= {this.props.Enter_amount}
  style={styles.phoneinput}
  autoCapitalize="none"
  // autoCorrect={false}
  autoCompleteType="email"
...
<TouchableOpacity activeOpacity={0.8} onPress={() => { this.check_previous_payout() }}
